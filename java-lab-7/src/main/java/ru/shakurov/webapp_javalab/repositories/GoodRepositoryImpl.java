...
import ru.shakurov.webapp_javalab.utils.RowMapper;
...
public class GoodRepositoryImpl implements GoodRepository, Component {
    ...
    private String FIND_ALL_BY_ID = "SELECT good.* FROM order_history LEFT JOIN good ON order_history.id_good=good.id WHERE order_history.id_user = ?";
    ...
    public List<Good> findAllById(Long id) {
        List<Good> goods = new ArrayList<>();
        try (PreparedStatement ps = connectionPool.getPooledConnection().getConnection().prepareStatement(FIND_ALL_BY_ID)) {
            ps.setLong(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    goods.add(rowMapper.rowMap(rs));
